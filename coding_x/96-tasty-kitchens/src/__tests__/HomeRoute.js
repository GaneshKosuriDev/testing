...
const server = setupServer(
  ...
  rest.get(restaurantsAPIURL, (req, res, ctx) => {
    ...
    console.log('offset', typeof offset, sortByRating)
    if (offset === '0' && sortByRating === 'Highest') {
      console.log('offset', offset, sortByRating)
...
describe.skip('Tasty Kitchen Application Authentication Home Route Tests', () => {
  beforeAll(() => {
    server.listen()
  })
  ...
  it.skip('When HTTP GET request should be made to offersApi is successful, the page should consist of at least two HTML list items, and the offers list should be rendered using a unique key as a prop for each similar offer item :::5:::', async () => {
    ...
    restoreGetCookieFns()
    await waitFor(() =>
      expect(screen.getAllByAltText('offer').length).toBeGreaterThanOrEqual(2),
    )
  ...
  it.skip('HomeRoute should consist of an HTML image element with the given logo URL as src and alt text as "website logo":::5:::', async () => {
    ...
    // await waitFor(() =>
    //   expect(screen.getByText(/Popular Restaurants/i, {exact: false})),
    // )
  ...
  it('HomeRoute should consist of an HTML list items :::5:::', async () => {
    mockRemoveCookie()
    mockGetCookie()
    rtlRender(<App />, homeRoutePath)
    ...
    // await waitFor(() => {
    //   expect(screen.getByText(/Cafe Madarassi/i))
    // })
